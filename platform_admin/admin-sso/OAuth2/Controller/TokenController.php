...
class TokenController implements TokenControllerInterface
{
    ...
    public function grantAccessToken(RequestInterface $request, ResponseInterface $response)
    {
        ...
        //TODO 注意新添加 当用户创建access_token时 同时需要为用户建立与oauth的绑定关系 创建用户在oauth平台的基本信息
        $Oauth2 = new Oauth2;
        $this->storage_redis = new Redis(Yii::$app->redis);
        $userid = $grantType->getUserId();

        if (!$userid) {
            $response->setError(400, 'error-userid', 'User information is lost when the token is created');//创建token 时用户信息丢失 所以创建token也失败

            return null;
        }

        $userinfo = $this->storage_redis->getPstuserByUseidFromUserinfo($userid);

        $Oauth2->setTposInfoFromDb($userinfo['uid'],$userinfo['client_id']);//获取token时需要建立用户的基本信息
