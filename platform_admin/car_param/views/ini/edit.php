            ...
            <form id="mymessage-form">
                ...
                <div class="col-md-12">
                    <div class="form-group col-md-6 input-xx">
                        ...
                        <select disabled name="category" class="form-control select2 select2-hidden-accessible col-md-9" style="width: 100%;" tabindex="-1" aria-hidden="true">
                            <?php foreach ($category as $item):?>
                                <option value="<?=$item['id']?>" <?php if($item['id']==$info['category_id']) echo 'selected';?>><?=$item['cnname']?></option>
                            <?php endforeach;?>
                        </select>
                    </div>
                </div>
                ...
                <div class="col-md-12">
                    <div class="form-group col-md-6 input-xx">
                        ...
                        <input name="filename" type="text" class="form-control my-colorpicker1 colorpicker-element bmdz" value="<?=$info['file_name']?>" autocomplete="off" placeholder="请输入ini文件名">
                    </div>
                </div>
                <div class="col-md-12">
                    <div class="form-group col-md-6 input-xx">
                        <label class='col-md-5 title'><?=Yii::t('app', 'INI_FILE_DESC')?>:</label>
                        <textarea type="textarea" class="form-control my-colorpicker1 colorpicker-element bmmc" autocomplete="off" name="desc" placeholder="请输入ini文件描述" ><?=$info['desc']?></textarea>
                    </div>
                </div>
                <div class="col-md-12">
                    <div class="form-group col-md-6 input-xx">
                        <label class='col-md-5 title'><?=Yii::t('app', 'INI_FILE_CONTENT')?>:</label>
                        <textarea type="textarea" class="form-control my-colorpicker1 colorpicker-element bmmc" autocomplete="off" name="content" placeholder="请输入ini文件内容" ><?=$info['ini_content']?></textarea>
                    </div>
                </div>
