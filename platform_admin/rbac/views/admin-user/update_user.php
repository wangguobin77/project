...
$AdminInstitutionsTypeInfo = (new AdminInstitutionsType)->disAdminInstitutionsTypeName();//组织
    ...
    <section class="content container-fluid">
        ...
        <div class="row col-md-12">
            <form id="admin-user-form">
                <input autocomplete="off"  name="_csrf" type="hidden" id="_csrf" value="<?= Yii::$app->request->csrfToken ?>">
                <input autocomplete="off"  name="id" type="hidden" value="<?=$model['id']?>">
                <input autocomplete="off"  type="hidden" name="work_number" value="<?=$model['work_number']?>">
                <!-- <div class="col-md-6 col-sm-12 col-xs-12"> -->
                <div class="col-md-6">
                    <div class="form-group col-md-12 input-xx">
                        <label class='col-md-3 title'>员工姓名:</label>
                        <input type="text" class="form-control my-colorpicker1 colorpicker-element ygxm" value="<?=$model['real_name']?>" autocomplete="off" name="real_name" placeholder="请输入员工姓名" >
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group col-md-12 input-xx">
                        <label class='col-md-3 title'>员工工号:</label>
                        <input type="text" class="form-control my-colorpicker1 colorpicker-element yggh" name="work_number" value="<?=$model['work_number']?>" disabled>
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    <div class="form-group col-md-12 input-xx">
                        ...
                        <input type="text" class="form-control my-colorpicker1 colorpicker-element sjhm" value="<?=$model['mobile']?>" autocomplete="off" name="mobile" placeholder="请输入手机号，同时用于账号登录">
                    </div>
                </div>
                <div class="col-md-6">
                    <div class="form-group col-md-12 input-xx">
                        ...
                        <select class="form-control select2 select2-hidden-accessible col-md-9" name="status" style="width: 100%;" tabindex="-1" aria-hidden="true">
                            <option <?=$model['status'] == 1?'selected':''?> value="1">在职</option>
                            <option <?=$model['status'] == 0?'selected':''?> value="0">离职</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    <div class="form-group col-md-12 input-xx">
                        ...
                        <input type="text" class="form-control my-colorpicker1 colorpicker-element bmmc laydate-icon" style="height:34px;border:1px solid #d2d6de;padding-left:12px;" name="bathday" value="<?=$model['birthday']?$model['birthday']:'' ?>" onclick="laydate({istime: false, format: 'YYYY-MM-DD',max:laydate.now()})">
                ...
                <div class="col-md-6">
                    <div class="form-group col-md-12 input-xx">
                        ...
                        <select name="sex"  class="form-control select2 select2-hidden-accessible col-md-9" style="width: 100%;" tabindex="-1" aria-hidden="true">
                            <option <?=$model['sex'] == 0?'selected':''?> value=0 >请选择</option>
                            <option <?=$model['sex'] == 1?'selected':''?> value=1>男</option>
                            <option <?=$model['sex'] == 2?'selected':''?> value=2>女</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    <div class="form-group col-md-12 input-xx">
                        ...
                        <input type="text" class="form-control my-colorpicker1 colorpicker-element sjhm" autocomplete="off" style="width: 100%;height:auto" id="dropdowntree" >
                        <div id="mm">
                    ...
                    <div class="form-group col-md-12 input-xx">
                        ...
                        <select name="position_id" class="form-control select2 select2-hidden-accessible col-md-9" style="width: 100%;" tabindex="-1" aria-hidden="true">
                            ...
                            <?php foreach ($positionList as $v):?>
                                <option <?=$model['position_id'] == $v['id']?'selected':''?> value="<?=$v['id']?>" ><?=$v['name']?></option>
                            <?php endforeach;?>
                        </select>
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    <div class="form-group col-md-12 input-xx">
                        ...
                        <textarea type="text" name="des" class="des form-control my-colorpicker1 colorpicker-element" autocomplete="off" placeholder="请输入备注信息" ><?=$model['des']?></textarea>
                    </div>
                </div>
        </div>
        <div class="row col-md-12">
...
<script type="text/javascript">
    ...
    var glo = {
        ...
        'select_org_json':null//已经选中的
    };

    $(document).ready(function(){

        glo.orgInfo = <?=$select_val?>;//所有部门组织信息
        glo.select_org_json = eval(<?=$userBranchList?>);//已经选中的组织或者部门信息


        //console.log(glo.select_org_json);
        ...
        //默认已经选中的元素
        dropdowntree.value(glo.select_org_json); //select items which hav
    });
    ...
    $('.submit-btn').click(function(){
        ...
        $.ajax({
            url:'<?=Url::toRoute('admin-user/update')?>',
            type:'post',
            dataType:'json',
            data:data,
