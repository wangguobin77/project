        ...
        <div id="sub-one">
            ...
            <div class="row info">
                <div class="form-group has-feedback col-md-8 col-xs-8">
                    <input type="mobile" class="form-control" name="yc_mobile" id="yc_mobile" value="<?=$yc_mobile?>" autocomplete="off" disabled>
                    <input type="hidden" class="form-control" name="mobile" id="mobile" value="<?=$mobile?>" autocomplete="off" disabled>
                    <span class="glyphicon glyphicon-user form-control-feedback"></span>
                    <!-- 提示信息 -->
                    <span class="sj-ts glyphicon glyphicon-remove-sign ts_mobile warning">手机号码不正确</span>
                </div>
                <button class="col-md-3 col-xs-4 fsyym verification cursor" id="code" style="cursor:pointer;">验证码</button>
            </div>
            <!-- 密码 -->
            <div class="row info">
                <div class="form-group has-feedback col-md-8 col-xs-8">
    ...
    $('.sub-two').click(function(){
        ...
        $.ajax({
            ...
            success:function (data) {
                ...
                if(data.code != 0){
                    //修改失败
                    // $('.success-box').find('h6').html('密码修改失败！');
                    // $('#sub-three').show();
                    succ('修改失败，请重新尝试！');
                }else{
                    //修改成功
                    ...
                    succ('密码修改成功！');
                    back_url('<?=Url::toRoute('index/index')?>');//参数为login的地址 跳回到登录
                }
