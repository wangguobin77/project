        ...
        <div class="right-box p-b-20 row">
            <ol class="breadcrumb">
                ...
                <span>
                    ...
                    <div class="right-box  lead-btn  import_btn" data-keymap="<?=$keymap_id?>" data-c="<?=$category_id?>" data-r="<?=$remote_type_id?>">
                        <button type="button" class="btn  btn-default" >
                            <?=Yii::t('app','IMPORT_COMMAND')?>
                        </button>
                    </div>

                    <form action="<?=Url::toRoute('keymap/add_keymap')?>" method="post">
                        <input name="_csrf" type="hidden" id="_csrf" value="<?= Yii::$app->request->csrfToken ?>">
                        <input type="hidden" name="remote_type_id" value="<?=$remote_type_id?>">
                        <input type="hidden" name="category_id" value="<?=$category_id?>">
                        <input type="hidden" name="keymap_id" value="<?=$keymap_id?>">
                        <input type="hidden" name="version" value="<?=$version?>">
                          <input type="hidden" name="k_type_id" value="2">
                        <button type="submit" type="submit" class="btn btn-success add-keymap"><?=Yii::t('app','ADD_KEYMAP')?></button>
                    </form>
                </span>
              </ol>
                    ...
                    <div class="zuoce-box col-md-2">
                        ...
                        <ul class='zuoce-box-list col-md-3'>
                            ...
                            <?php foreach($keymap_data_list as $key=>$val):?>
                                <?php if($val['id'] == $id):?>
                                    <li class='sl ml-item li-active'><a href="<?=Url::toRoute(['keymap/detail_view','keymap_id'=>$keymap_id,'id'=>$val['id'],'request_type'=>2])?>" >
                                            <?=Yii::t('db',$val['command'])?></a><span class="btn-del" data-id="<?=$val['id']?>" data-keymap-id="<?=$keymap_id?>">删除</span></li>
                                <?php else:?>
                                    <li class='sl ml-item'><a href="<?=Url::toRoute(['keymap/detail_view','keymap_id'=>$keymap_id,'id'=>$val['id'],'request_type'=>2])?>" >
                                            <?=Yii::t('db',$val['command'])?></a><span class="btn-del" data-id="<?=$val['id']?>" data-keymap-id="<?=$keymap_id?>">删除</span></li>
                                <?php endif;?>
                            <?php endforeach;?>
                        </ul>
                        ...
                        <div class="youce-box youce-detail-box col-md-9">
                            ...
                            <div class='ter-box'>
                                <div class="mlbj-xx">
                                    ...
                                    <select  name="COMMAND" class="sl" id="command" data-v="<?=$keymap_id?>" >
                                ...
                                <div class="mlbj-xx">
                                    ...
                                    <select name="KEYMAP_TYPE" class="sl" id="keymap_type" >
                                        <option value="0">--<?=Yii::t('app','PLEASE_CHOOSE')?>--</option>
                                        <?php
                                ...
                                <div class="mlbj-xx select-mlxx-box add-box">
                                    ...
                                    <span class='font_family icon-operation_delate del' data-toggle="tooltip" data-placement="top" title="默认删除最后一条" style="font-size:24px;cursor:pointer;"></span>
                                </div>
                                <!-- 正常参数 -->
                                <!--模拟量参数-->
                                <!--模拟量参数-->
                                <?php if(isset(json_decode($data['km_data'],true)['PARAMS'])&& count(json_decode($data['km_data'],true)['PARAMS']) > 0){
                                $str_a = '';//不存在操作手
                                $str_u ='';//美国手
                                $str_j = '';//日本手
                                $str_c = '';//中国手
                                    ...
                                    <?php if(isset($jsonData['PARAMS'])&& count($jsonData['PARAMS']) > 0){
                                        foreach ($jsonData['PARAMS'] as $key=>$val){
                                            if($jsonData['PARAMS'][$key]['TYPE'] == 'ANALOG'){
                                                ...
                                                //$str_m .= '<input type="text"  class="sl itemcs" value="'.Yii::t('db',$val['VALUE']).'"/>';//2019-1-09
                                                $str_m .= '<input type="text"  onblur="very_tc(this)" name="PARAMS['.$key.'][TYPE][TRANSPARENT]" class="sl itemcs" value="'.Yii::t('db',$val['VALUE']).'"/>';
                                            }
...
<!-- 导入命令弹框 -->
<div class='call-box have_command' style="display:none;">
    <div class='call-box-son'>
        <div class="commond-box commond-have-box">

            <span class="close font_family icon-close fa fa-close"></span>
            <h4>导入命令</h4>
            <form id="import_form">
                <input name="_csrf" type="hidden" id="_csrf" value="<?= Yii::$app->request->csrfToken ?>">
                <input type="hidden" name="keymap_id" value="<?=$keymap_id?>">
                <div class="commond-list">
                    <div class="xx">
                        <div class="type">
                            <h4 class="com_name"></h4>
                        </div>
                        <ul>
                            <li class="li_list">
                ...
                <!--<button type="button" class="have-button lead-confirm">确定</button>-->
                <a class="import_btn_submit lead-confirm noselect btn btn-primary"><?=Yii::t('app','CONFIRM')?></a>
            </form>
...
<script type="text/javascript">
    ...
    $('.back').click(function(){
        window.location.href = '<?=Url::toRoute('keymap/keymap_list')?>';
    });
    //删除命令
    $('.btn-del').click(function(){

        if(!glo.is_true) return;
        var id = $(this).attr('data-id');
        var keymap_id = $(this).attr('data-keymap-id');
        ...
        $('.del-box').find('.confirm').unbind('click').click(function(){
            ...
            glo.on_off = false;
            $.ajax({
                url:'<?=Url::toRoute('keymap/del_keymap_data')?>',
                type:'post',
                dataType:'json',
                data:{'id':id,'keymap_id':keymap_id,'_csrf':'<?= Yii::$app->request->csrfToken?>'},
                success:function (data) {
                    glo.on_off = true;
                    if (data.code == 0 ){
                        succ(data.message,'<?=Url::toRoute(['keymap/detail_view','keymap_id'=>$keymap_id,'request_type'=>1])?>');
                    }else{
                        fail(data.message);
                    }
    ...
    $('.import_btn').click(function(){
        var category_id = $(this).attr('data-c');
        var remote_type_id = $(this).attr('data-r');
        var _csrf = '<?= Yii::$app->request->csrfToken ?>';
        $.ajax({
            url: '<?=Url::toRoute('keymap/import_command')?>',
            type: 'post',
            dataType: 'json',
            data: {'category_id':category_id,'remote_type_id':remote_type_id,'_csrf':_csrf},
            success:function (data) {
                console.log(data);
                if(data.code==0){

                    var res = data.data.cm_r;
                    if(res.length <= 0){
                        return false;
                    }
                    $('.com_name').html('<?=Yii::t('app','OFFICLE_VER')?>:'+'<?=Yii::t('db',$c_type)?Yii::t('db',$c_type):''?>:V'+data.data.ver);
                    var str = '';
                    for(var i = 0;i<res.length;i++){
                       /* str += '<a><input type="checkbox" name="command[]['+res[i].command+']" value='+res[i].km_data+'><h6 class="sl">'+res[i].command+'</h6></a>';*/
                        str += '<a><input type="checkbox" name="command[]['+res[i].command+']" value='+res[i].km_data+'><h6 class="sl">'+res[i].language_command+'</h6></a>';
                    }
                    $('.li_list').html(str);
                    // alert('dads');
                    $('.have_command').css('display','block');
                }else{
                    fail('没有可导入的命令！');
                }
    ...
    //倒入命令
    $('.import_btn_submit').click(function(){

        if(!glo.is_true) return;
        var v = false;

        // for(i=0;i<$('.li_list input[type="checkbox"]').length;i++){
        //     if($('.li_list input[type="checkbox"]').get(i).checked){
        //         v = true;
        //     }else{
        //         fail('请选择命令！');
        //     }
        ...
        if(v){
            ...
            glo.is_true = false;
            $.ajax({
                url: '<?=Url::toRoute('keymap/dis_import_command')?>',
                type: 'post',
                dataType: 'json',
                data: data,
                success:function (data) {
    ...
    $('.ml-item').click(function(){
        ...
        $('.add-commond').removeClass('add-ml-active');
        $('.youce-add-box').hide();
...
//            par_str +=  '<h4 style="position:relative"><?//=Yii::t('app','SIMULATION_PARAMETERS')?>//<span class="cdcs simulation"></span></h4>';
    ...
    function getAllkeyEvent()
    {
        ...
        if(!keymap_id){
            fail('please again create keymap');
            return false;
        }
        if(!category_id){
            fail('please again create keymap');
