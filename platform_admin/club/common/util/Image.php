...
abstract class Image {
     ...
     */
    public static function instance($file, $driver = NULL)
    {
        if ($driver === NULL)
        {
    ...
    public function save($file = NULL, $quality = 100)
    {
        ...
        if (is_file($file))
        {
            if ( ! is_writable($file))
            {
                throw new \Exception('File must be writable: '.$file, E_ERROR);
            }
        }
        else
        {
            ...
            {
                throw new \Exception('Directory must be writable: '.$directory, E_ERROR);
            }
        }
