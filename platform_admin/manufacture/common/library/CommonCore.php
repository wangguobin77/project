...
class CommonCore
{
    ...
    /**
     * 多种注册方式 字段值都是唯一索引 所以值不能为空 默认值都统一
     * @param $data array
     * 1 用户名类型
     * 2 用户邮箱类型
     * 3 用户手机类型
     * 4 第三方类型
     * @return array  data 原始数据   regtype 注册来源
     *
     */
    public function setAllUserNameUnify($data)
    {
        $userName = '';
        $regType = 0;
        ...
        if($data && isset($data['useremail'])){
            $userName = $data['useremail'];
            $regType = 2;
        }
        if($data && isset($data['usermobile'])){
            $userName = $data['usermobile'];
            $regType = 3;
        }
        //扩展字段 都默认 为用户名注册
        if($data && isset($data['standby_name1'])){
            $userName = $data['standby_name1'];
            $regType = 1;
        }
        if($data && isset($data['standby_name2'])){
            $userName = $data['standby_name2'];
            $regType = 1;
        }
        $data['username'] = $userName;
        $data['useremail'] = $userName;
        $data['usermobile'] = $userName;
        $data['standby_name1'] = $userName;
        $data['standby_name2'] = $userName;

        $data['nickname'] = $userName;//不能为空

        return array('data'=>$data,'retype'=>$regType);
    }
     ...
     * 添加 推送用手机验证码 和邮箱链接 到客户端 成功或者失败log
     */
    public static function add_manufacture_register_verify_log($data)
    {
        $model = new ManufactureRegisterVerifyLog();
        $res = $model->add($data);

        if( $res['@ret']!=1 ){
            var_log($data,'add_manufacture_register_verify_log');
        }
    }
}