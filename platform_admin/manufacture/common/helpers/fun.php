...
/**
 * 用户操作日志(谁 某时 做了什么事)
 * @param string $act  操作事件 如delete update...
 * @param string $des  描述
 * @param string $sql
 */
function insert_db_log($act,$des='', $sql=''){
    if($sql != ''){
        \events\AdminLogEvent::$sqlarr[] = $sql;
    }
    call_user_func(['\events\AdminLogEvent', 'insert'], func_get_args());
}
