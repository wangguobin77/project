...
class UploadImage
{
     ...
     * @return array
     */
    public function upload($img,$dir,$filename='',$width=300,$height=300)
    {
        ...
        //图片路径地址
        $basedir = Yii::$app->basePath.$dir;

        $http_small_url = $dir.'/'.$small; //http 访问图片路径
        ...
        $types = empty($types)? ['jpeg' ,'gif', 'png', 'pjpeg', 'x-png', 'bmp'] :$types;
        ...
        if(preg_match('/^(data:\s*image\/(\w+);base64,)/', trim($img), $matches)){
            ...
            $photo = $filename ? '/'.$filename.'.'.$type : '/'.createGuid().'.'.$type;
            ...
            //创建对应的缩略图
            $this->CreateThumbnail($fullpath.$photo,$smallpath.$photo,$width,$height);
            return $ary;
