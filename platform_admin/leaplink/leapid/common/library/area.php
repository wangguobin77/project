<?php
/**
 * 获取全国地址
 * Created by PhpStorm.
 * User: localuser1
 * Date: 2018/8/9
 * Time: 下午2:26
 */

namespace common\library;

use yii;
use backend\models\ShopAreas;
class area
{
    protected static $config = array(
        /* 主键 */
        'primary_key' 	=> 'area_id',
        /* 父键 */
        'parent_key'  	=> 'parent_id',
        /* 展开属性 */
        'expanded_key'  => 'expanded',
        /* 叶子节点属性 */
        'leaf_key'      => 'leaf',
        /* 孩子节点属性 */
        'children_key'  => 'children',
        /* 是否展开子节点 */
        'expanded'    	=> false
    );

    /* 结果集 */
    protected static $result = array();

    /* 层次暂存 */
    protected static $level = array();

    /**
     * @brief 获取地区
     */
    public function areaChild()
    {
        $areaInfo = ShopAreas::find()->asArray()->all();

        return self::makeTree($areaInfo);
        /*$fh = fopen(ROOT_PATH.'js/public/area.js','w');
        fwrite($fh,json_encode(self::makeTree($areaInfo)));
        fclose($fh);*/

    }

    /**
     * @name 生成树形结构
     * @param array 二维数组
     * @return mixed 多维数组
     */
    public static function makeTree($data,$options=array() ){

        $dataset = self::buildData($data,$options);

        $r = self::makeTreeCore(0,$dataset,'normal');
        return $r;
    }

    /* 生成线性结构, 便于HTML输出, 参数同上 */
    public static function makeTreeForHtml($data,$options=array()){

        $dataset = self::buildData($data,$options);
        $r = self::makeTreeCore(0,$dataset,'linear');
        return $r;
    }

    /* 格式化数据, 私有方法 */
    private static function buildData($data,$options){
        $config = array_merge(self::$config,$options);
        self::$config = $config;
        extract($config);
        $r = array();
        foreach($data as $item){
            $id = $item[$primary_key];
            $parent_id = $item[$parent_key];
            $r[$parent_id][$id] = $item;
        }
        return $r;
    }


    /* 生成树核心, 私有方法  */
    private static function makeTreeCore($index,$data,$type='linear')
    {
        //判断data 索引值是否存在 不存在就登陆不了
        if(!isset($data[$index]) || !$data[$index]){
            show_json(100111,'Need to contact the administrator reinstall authority');//需要联系管理员 从新设置权限
        }
        extract(self::$config);
        foreach($data[$index] as $id=>$item)
        {
            if($type=='normal'){

                if(isset($data[$id]))
                {
                    $item[$expanded_key]= self::$config['expanded'];
                    $item[$children_key]= self::makeTreeCore($id,$data,$type);
                }
                else
                {
                    $item[$leaf_key]= true;
                }
                $r[$item['area_id']] = $item;
            }else if($type=='linear'){
                $parent_id = $item[$parent_key];
                self::$level[$id] = $index==0?0:self::$level[$parent_id]+1;
                $item['level'] = self::$level[$id];
                self::$result[] = $item;
                if(isset($data[$id])){
                    self::makeTreeCore($id,$data,$type);
                }

                $r = self::$result;
            }
        }
        return $r;
    }
}